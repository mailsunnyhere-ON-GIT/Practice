...
fetch("https://dummyjson.com/products").then((res)=>res.json()).then((res)=>{
    const mappedProducts = res.products.map((one)=>{
        console.log(one);
        return one; // Return each product if needed
    });
    return mappedProducts; // Return the mapped array if needed
});

},{}]},["km5uZ","1xC6H","bB7Pu"], "bB7Pu", "parcelRequire998b")
